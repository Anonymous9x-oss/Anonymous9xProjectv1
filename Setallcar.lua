...
if _G.AnonymousWalkInstance then
	-- ========== MODE OFF ==========
	-- hancurin script lama (berhenti total)
	local oldScript = _G.AnonymousWalkInstance
	if oldScript and oldScript.Parent then
		oldScript:Destroy()
	end
	_G.AnonymousWalkInstance = nil
	_G.WalkActive = false
	...
	return  -- stop eksekusi script yang baru (biar gak jalan ulang)
else
	-- ========== MODE ON ==========
...
function _Popper()
	...
	local function queryPoint(origin, unitDir, dist, lastPos)
		...
		--repeat -- this is the most stupidest thing I've ever seen today
			local success, entryPart, entryPos = pcall(function()
				return workspace:FindPartOnRayWithIgnoreList(ray(movingOrigin, target - movingOrigin), blacklist, false, true)
			end)
			if success and entryPart then
				if canOcclude(entryPart) then
					...
					if exitPart then
						local promote = false
						if lastPos then
							local _, promote2 = pcall(function()
								return workspace:FindPartOnRayWithWhitelist(ray(lastPos, target - lastPos), wl, true)
							end)
							promote = promote2
							if not promote2 then
								local _, promote2 = pcall(function()
									return workspace:FindPartOnRayWithWhitelist(ray(target, lastPos - target), wl, true)
								end)
								promote = promote2
							end
						end
...
function _GravityController()
	...
	function GravityController:OnHeartbeatStep(dt)
		...
		local success, hit, pos, normal = pcall(function()
			return workspace:FindPartOnRayWithIgnoreList(ray, self.Ignores)
		end)
		local lastPart = self.LastPart

		if (success and hit and lastPart and lastPart == hit) then
