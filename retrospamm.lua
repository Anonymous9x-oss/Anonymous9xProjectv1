...
-- Emoticon pool
local EMOTES = {
    ":333","xD","OwO","UwU",">_<","^_^","T_T",";3",
    "x_x","XD",":P","rawr","rawrrr","rawer",">w<",
    "qwq","ono","unu"," :3 ","X3","ツ",
}
...
local function slopify(str)
    ...
    -- Step 2: slang swap (whole-word pattern)
    for word, replacement in pairs(SLANG) do
        s = s:gsub("%f[%a]" .. word .. "%f[%A]", replacement)
    end

    -- Step 3: leetspeak substitution char by char
    local leet = {}
    for i = 1, #s do
        local ch = s:sub(i, i)
    ...
    -- Step 4: alternating caps (50% per character)
    local alt = {}
    for i = 1, #s do
        local ch = s:sub(i, i)
        if ch:match("%a") then
    ...
    -- Step 5: append 4-8 exclamation marks
    local excl = math.random(4, 8)
    s = s .. string.rep("!", excl)

    -- Step 6: append random emoticon
...
-- ══════════════════════════════════════
-- CHAT SENDER (universal FE)
-- ══════════════════════════════════════
local function sendChat(msg)
    -- Method 1: TextChatService (new API)
    pcall(function()
    ...
    -- Method 2: Legacy chat system fallback
    pcall(function()
...
pcall(function()
    local old = LP.PlayerGui:FindFirstChild("RetroslopGUI")
    if old then old:Destroy() end
end)
pcall(function()
    local old = game.CoreGui:FindFirstChild("RetroslopGUI")
    if old then old:Destroy() end
end)
...
Panel.Active             = true
Panel.Draggable          = true   -- native drag (PC)
Panel.ZIndex             = 10
Panel.Parent             = ScreenGui
Instance.new("UICorner", Panel).CornerRadius = UDim.new(0, 10)

-- Border stroke (animated)
...
-- ── BORDER GLOW LOOP ───────────────────────────
-- Smoothly oscillates between white and purple
task.spawn(function()
    local t = 0
    while Panel.Parent do
        t = t + task.wait(0.04)
        local s = (math.sin(t * 1.8) + 1) / 2   -- 0 → 1
        BorderStroke.Color = WHITE:Lerp(PURPLE, s)
        BorderStroke.Thickness = 1.2 + s * 0.9
    end
end)

-- ── ENTRANCE ANIMATION (Elastic scale-in) ──────
...
Subtitle.Text              = "retroslop + spam 5x"
...
-- Minimize logic (toggle content visibility)
...
InputBox.FocusLost:Connect(function()
    TweenService:Create(InputStroke, TweenInfo.new(0.15), {Transparency = 0.35}):Play()
end)
...
SpamBtn.MouseEnter:Connect(function()
    TweenService:Create(SpamBtn, TweenInfo.new(0.12), {BackgroundColor3 = PURPLE_BRIGHT}):Play()
end)
SpamBtn.MouseLeave:Connect(function()
    TweenService:Create(SpamBtn, TweenInfo.new(0.12), {BackgroundColor3 = PURPLE}):Play()
end)
...
local function showEmpty()
    ...
    task.delay(1.0, function()
        TweenService:Create(EmptyNotif, TweenInfo.new(0.35), {TextTransparency = 1}):Play()
    end)
end

-- ── DRAG SUPPORT (Mobile / UIS) ────────────────
-- Supplements the native .Draggable for mobile touch
do
...
SpamBtn.MouseButton1Click:Connect(function()
    ...
    -- Disable button visually during spam
    SpamBtn.AutoButtonColor = false
    SpamBtn.BackgroundColor3 = Color3.fromRGB(100, 30, 160)
    SpamBtn.Text = "Sending..."

    -- Show and animate progress bar (3 seconds full fill)
    ProgressBG.Visible = true
    ProgressFill.Size  = UDim2.new(0, 0, 1, 0)
    local progTween = TweenService:Create(
        ProgressFill,
        TweenInfo.new(3.0, Enum.EasingStyle.Linear),
        { Size = UDim2.new(1, 0, 1, 0) }
    )
    progTween:Play()

    -- Send 5 slopified messages with 0.6s delay
    task.spawn(function()
        for i = 1, 5 do
